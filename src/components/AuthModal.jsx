    ...
    if (isOpen && (mode === 'login' || mode === 'register')) {
      ...
      const tryRender = () => {
        ...
        if (container) {
          ...
          if (!ok && retries < 20) {
          ...
          } else if (!ok && container && !container.hasChildNodes()) {
            // Render mobile-friendly fallback button if GIS SDK failed to load
            container.innerHTML = `
              <button type="button" class="w-[280px] py-2.5 px-4 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-full font-bold text-xs text-slate-700 dark:text-slate-200 shadow-sm flex items-center justify-center gap-2 hover:bg-slate-50 transition-colors">
            ...
            if (btn) {
              btn.onclick = () => {
                const retryOk = realGoogleAuthService.renderOfficialGoogleButton(container, handleGoogleSignIn);
                if (!retryOk) {
                  realGoogleAuthService.promptOneTap(handleGoogleSignIn);
                }
              };
            }
          }
